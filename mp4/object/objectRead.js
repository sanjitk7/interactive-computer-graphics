// function for setting the flag that picks the shader in case color input is required (6 vertex parameter case)
function checkObjHasColor(text) {
  let read_lines = text.split("\n");
  for (let i = 0; i < read_lines.length; i++) {
    let words = read_lines[i].split(" ").filter(function (e) {
      return e;
    });
    if (words[0] == "v") {
...
async function objectToGeom(text) {
  ...
  for (let i = 0; i < read_lines.length; i++){
      ...
      if (read_words[0]=="f"){
          ...
          if ((read_words.length-1) > 3 ) {
              ...
              // for normal and tex coord case - group normals and textures
              if (current_line.includes("/")){
                  for (let j=0; j<combination_triangles.length; j++){
                      gl_vertex_position = []

                      for (let x=0; x<3 ; x++){
                          temp_vertex = parseFloat(combination_triangles[j][x].split("/")[0])
                          temp_texture = parseFloat(combination_triangles[j][x].split("/")[1])
                          temp_normal = parseFloat(combination_triangles[j][x].split("/")[2])

                          if (temp_normal !== "") objGeom.attributes.normal[temp_vertex - 1] = normal[temp_normal - 1]
                          if (temp_texture!=="") objGeom.attributes.texture[temp_vertex - 1] = texture[temp_texture - 1]
